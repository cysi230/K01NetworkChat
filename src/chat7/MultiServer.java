	...
	class MultiServerT extends Thread{
		...
		try {
			...
			System.out.println("현재 접속자수는"
				...
				/*
				 클라이언트가 전송한 메세지가 명렁어인지 판단한다.
				 */
				if(s.charAt(0)=='/') {
					//만약  (/)로 시작한다면 명령어이다
					/*
					 귓속말인은 아래와 같이 전송하게 된다
					 => /to [대화명] [대화내용]
					 	:따라서 split()을 통해 space문자열을 분리한다
					 */
					...
					if(strArr[0].equals("/to")) {
						//
						//함수 호출시 One이면 한명에게만 메세지전송
						 sendAllMsg(strArr[1], msgContent,"One");
					}
				}
				else {
					//만약 (/)로 시작하지않으면 일반 메세지이다.
					sendAllMsg(name,s,"All");//All이면 전제전송
				}				
			}
		} catch (Exception e) {
